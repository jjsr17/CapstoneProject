...
export default function AccountScreen() {
  ...
  // load logged-in user id from storage
  useEffect(() => {
    (async () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const data = await gqlFetch(SESSIONS_BY_STUDENT, { studentId });
        if (!alive) return;

        setSessions(data?.sessionsByStudent ?? []);
      } catch (e) {
        if (!alive) return;
  ...
  if (!studentId) return <Text style={{ marginTop: 50, padding: 16 }}>Not logged in yet.</Text>;
